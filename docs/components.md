...
| `ServiceFormDialog` | `catalog/service-form-dialog.tsx` | Client. Create/edit catalog entry (name, description, category, measure, duration). Create mode has a price field (feeds the Estándar variant). Duration services hide the stock checkbox (forced `tracksStock:false`) and render `VariantManager` with `kind="service"`. Props `service?`, `categories`, `mode?`. POST/PUT `/api/services`. |
...
| `ProductCard` | `store/product-card.tsx` | Server-safe. Store listing card linking to `/store/:salonId/:itemId`. Props `item`, `currency`, `salonId`, `view?` (`grid` square card / `list` horizontal row), `categoryName?` (list view). "desde" price with >1 variant, "Agotado" badge + faded cover when tracked stock is 0. Duration services show "/hora" (per_unit) and a "~N min" line. |
...
| `StoreBrowser` | `store/store-browser.tsx` | Client. Filtered store listing: `StoreFilters` + `ViewToggle` + result count + grid/list of `ProductCard`; empty state with "Limpiar filtros". Filter state lives in URL params `q,cat,min,max,stock,type,view` (shallow). Type filter only offered when the catalog has both products and services. Props `items`, `categories`, `currency`, `salonId`. |
| `StoreFilters` | `store/store-filters.tsx` | Client. Productos/Servicios chips (when `showTypeFilter`), category Select, min/max price inputs, "Solo disponibles" chip, "Limpiar". Writes params via `setParams` (shallow). Collapsed behind "Filtros" button (+active count badge) below `sm`. Props `categories`, `showTypeFilter?`. |
| `ViewToggle` | `store/view-toggle.tsx` | Client. Grid/list icon buttons; persists `view` URL param (grid = param removed). No props. |
| `ProductDetail` | `store/product-detail.tsx` | Client. Store detail orchestrator: variant + active-photo state, `ProductGallery`, price, `VariantPicker` (hidden with ≤1 variant; single variant auto-selected; labeled "Tarifas" for services), low-stock warning (1–5), duration line and "/hora" for services, description, `SelectionSummary`. With "Todas" active, the summary resolves the variant owning the chosen photo (exact price instead of "desde"). 3-col grid on `lg`, stacked on mobile. Props `item`, `currency`. |
...
| `SelectionSummary` | `store/selection-summary.tsx` | Presentational. Detail-page side panel ("Tu selección"): active photo thumb, variant/tarifa (hidden with ≤1 variant), price (+"/hora"), availability for products or duration for services. Informational only — no cart. Sticky on `lg`. Props `item`, `variant?`, `photo?`, `currency`. |
...
| `SettingsForm` | `settings/settings-form.tsx` | Client. Edit salón settings (currency, tax %, timezone, address, phone, logo). Logo: preview + file upload (POST `/api/salon-settings/logo`, persists immediately) + Quitar (DELETE) or manual external URL (saved with the form). Prop `settings?`. PUT `/api/salon-settings`. |
