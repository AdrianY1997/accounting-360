...
| `SelectionSummary` | `store/selection-summary.tsx` | Presentational. Detail-page side panel ("Tu selección"): active photo thumb, variant/tarifa (hidden with ≤1 variant), SKU (variant's, falls back to item's), price (+"/hora"), availability for products or duration for services. Informational only — no cart. Sticky on `lg`. Props `item`, `variant?`, `photo?`, `currency`. |
